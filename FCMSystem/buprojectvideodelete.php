...
if (isset($_GET["deletevideo"])) {
    ...
    $getvifilenameQuery = "SELECT vifilename FROM res_video_db WHERE viid = '$id'";
    ...
    if ($getvifilenameResult && mysqli_num_rows($getvifilenameResult) > 0) {
        ...
        $deleteRecordQuery = "DELETE FROM res_video_db WHERE viid = '$id'";
        $deleteRecordResult = mysqli_query($conn, $deleteRecordQuery);

        if ($deleteRecordResult) {
            // Deletion of record successful, now delete the video file
