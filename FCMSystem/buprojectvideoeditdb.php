...
if (isset($_POST['editvideo'])) {
    ...
    $oldvifilename = $_POST['oldvifilename'];

    // UPDATE table_name SET column1=value1, column2=value2 WHERE condition
    $query = "UPDATE res_video_db SET prid = '$prid', vititle = '$vititle' WHERE viid = '$id'";
    $data = mysqli_query($conn, $query);

    // Check if a file was uploaded
    if ($_FILES['vicontent']['size'] > 0) {
        ...
        $vifilename = moveUploadedFile($_FILES["vicontent"], $uploadDirectory, $allowedVideoExtensions);

        if ($vifilename !== false) {
            // Update the database with the new Video filename
            $updateFilenameQuery = "UPDATE res_video_db SET vifilename = '$vifilename' WHERE viid = '$id'";
            mysqli_query($conn, $updateFilenameQuery);
            header("Location: http://localhost/FCMS/FCMSystem/buprojectvideo.php");
            echo "Video uploaded and updated successfully.";
        } else {
