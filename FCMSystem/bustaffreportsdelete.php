...
if (isset($_GET["deletetaskreport"])) {
    // Retrieve the ID from the form
    $id = $_GET["deletetaskreport"];

    // Perform the deletion query
    $deleteQuery = "DELETE FROM res_taskreport_db WHERE trpid = '$id'";
    $deleteResult = mysqli_query($conn, $deleteQuery);

    if ($deleteResult) {
        // Deletion successful
        header("Location: http://localhost/FCMS/FCMSystem/bustaffreports.php");
    } else {
